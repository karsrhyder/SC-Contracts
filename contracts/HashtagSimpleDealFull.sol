...
contract HashtagSimpleDeal is Ownable {
    ...
    constructor(address _token, string memory _hashtagName, uint _hashtagFee, bytes32 _hashtagMetadataHash, address _seekerRep, address _providerRep) public {

        /// @notice The name of the hashtag is set
        hashtagName = _hashtagName;

        /// @notice If the constructor param seekerRep = 0x0
        /// @notice The seeker reputation token is created
        //if(_seekerRep == 0x0) {
          SeekerRep = new DetailedERC20(_hashtagName, "SWRS", 0);
        //}

        // if(_seekerRep != 0x0) {
        //   SeekerRep = DetailedERC20(_seekerRep);
        // }  

        /// @notice If the constructor param seekerRep = 0x0
        /// @notice The provider reputation token is created
        //if(_providerRep == 0x0) {
          ProviderRep = new DetailedERC20(_hashtagName, "SWRP", 0);
        //}

        // if(_providerRep != 0x0) {
        //   ProviderRep = DetailedERC20(_providerRep);
        // }  
    ...
    function receiveApproval(address _msgsender, uint _amount, address _fromcontract, bytes memory _extraData) public {
        require(address(this).call(_extraData), "Err");
    ...
    function transferRepOwnership(address _newaddress) public onlyOwner {
        SeekerRep.transferOwnership(_newaddress);
        ProviderRep.transferOwnership(_newaddress);
        emit HashtagChanged("Reptoken Owner transferred");
    }
    ...
    function payoutItem(bytes32 _itemHash) public {
        ...
        /// @dev pay out the maintainer
        require(token.transfer(payoutAddress, c.hashtagFee), "Error transfering funds");
    ...
    function cancelItem(bytes32 _itemHash) public {
        itemStruct storage c = items[_itemHash];
        if(c.itemValue > 0 && c.providerAddress == address(0x0) && c.status == itemStatuses.Open) {
            /// @dev pay out the maintainer
            require(token.transfer(payoutAddress, c.hashtagFee / 2), "Error transfering funds");
    ...
    function disputeItem(bytes32 _itemHash) public {
        ...
        if (msg.sender == c.seekerAddress) {
            ...
            require (c.providerAddress != address(0x0), "provider not 0 not open");
        } else {
    ...
    function getitemHashes() public view returns(bytes32[] memory) {
        return itemHashes;
    }
    ...
    function getItemReplies(bytes32 _itemHash) public view returns(bytes32[] memory) {
        return items[_itemHash].replies;
    }

    function getItemRepliers(bytes32 _itemHash) public view returns(address[] memory) {
